...
CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags="-X $flags_path.BuildVersion=$BUILD_VERSION -X $flags_path.Commit=$COMMIT" -o tmp/upload/wekactl_linux_arm64 cmd/wekactl/*.go
CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags="-X $flags_path.BuildVersion=$BUILD_VERSION -X $flags_path.Commit=$COMMIT" -o tmp/upload/wekactl_darwin_arm64 cmd/wekactl/*.go
